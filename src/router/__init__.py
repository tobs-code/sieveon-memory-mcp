...
from .policy import BudgetLevel, OverBudget, QueryType, RoutingPolicy
...
class Router:
    ...
    def route(
        ...
        strategy, budget, policy_applied = self.routing_policy.get_strategy(
    ...
    def execute_with_budget(self, query_type: QueryType, operation_fn, **budget_kwargs):
        ...
        try:
            ...
            strategy, _, _ = self.routing_policy.get_strategy(
                query_type, 0.5
            )  # Default confidence
            cost_tracker.record_request(
                strategy=strategy,
...
def classify_query(text: str) -> Tuple[QueryType, float]:
    ...
    if any(
        word in text_lower
        for word in ["when", "time", "date", "before", "after", "during"]
    ):
        return QueryType.TEMPORAL, 0.8
    elif any(
        word in text_lower for word in ["what", "who", "where", "how much", "which"]
    ):
        return QueryType.FACTUAL, 0.7
    elif any(
        word in text_lower
        for word in ["relationship", "connected", "related to", "connection"]
    ):
        return QueryType.MULTI_HOP, 0.8
    elif any(
        word in text_lower for word in ["conversation", "chat", "talk", "discuss"]
    ):
        return QueryType.CONVERSATIONAL, 0.6
    elif any(
        word in text_lower for word in ["update", "change", "modify", "edit", "correct"]
    ):
