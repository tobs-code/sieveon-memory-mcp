...
class STRATAEvalHarness:
    def __init__(self):
        ...
        self.ns = os.getenv("SURREALDB_NS", "strata")
        self.db = os.getenv("SURREALDB_DB", "strata")
    ...
    def execute_strategy(self, strategy_name, query):
        ...
        if isinstance(result, list):
            for r in result:
                if isinstance(r, list) and len(r) > 1 and isinstance(r[1], dict) and r[1].get("result"):
                    success = True
                    break
        elif isinstance(result, list) and len(result) > 1 and isinstance(result[1], dict) and result[1].get("result"):
            success = True
        
        return result, latency, num_queries, success

    def evaluate(self, test_cases):
        print("Running STRATA Evaluation Harness...")
        ...
        results["metrics"]["long_horizon_stability"] = self.measure_long_horizon_stability()
    ...
    def print_summary(self, results):
        print("\n" + "="*50)
        print("STRATA EVALUATION RESULTS")
        print("="*50)
...
if __name__ == "__main__":
    harness = STRATAEvalHarness()
    results = harness.run_all_evals()
    
    # The summary is already printed in run_all_evals
    # Save results to file
    with open("strata_eval_results.json", "w") as f:
        json.dump(results, f, indent=2)
    print("Results saved to strata_eval_results.json")